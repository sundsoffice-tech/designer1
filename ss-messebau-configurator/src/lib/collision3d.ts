...
export const colliderBoundsToAabb = (
  ...
  return {
    id,
    label,
    centerX: (baseBox.min.x + baseBox.max.x) / 2,
    centerZ: (baseBox.min.z + baseBox.max.z) / 2,
    halfWidth: (baseBox.max.x - baseBox.min.x) / 2,
    halfDepth: (baseBox.max.z - baseBox.min.z) / 2,
    baseWidth: baseBox.max.x - baseBox.min.x,
    baseDepth: baseBox.max.z - baseBox.min.z,
    clearance,
    rotationY: 0,
    minX: baseBox.min.x,
    maxX: baseBox.max.x,
