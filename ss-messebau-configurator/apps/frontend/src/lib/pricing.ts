...
// Alle Domain-Typen kommen aus @ss/shared (Single Source of Truth)
export type {
  StandType, Region, WallSide, WallType, WallSurface, WallConfig, WallDetailConfig,
  WallPanelRules, WallPanelConfig, WallAttachmentBinding, WallAttachmentIndex,
  FloorType, FloorConfig, CabinDoorSide, CabinDoorConfig, CabinConfig,
  ScreenSize, ScreenConfig, SeatingType, SeatingCover, SeatingConfig,
  ChairConfig, CustomObjectConfig, LampConfig, RoundTableConfig,
  TrussLightConfig, WallLightConfig, CounterPlacement, CounterVariant,
  CounterConfig, StandModules, StandConfig,
} from "@ss/shared";

import type {
  Region, WallSide, WallType,
  FloorType, CounterVariant, ScreenSize,
  SeatingType, SeatingCover, SeatingConfig,
  ScreenConfig, ChairConfig, CounterConfig, CustomObjectConfig,
  RoundTableConfig, StandModules, StandConfig,
} from "@ss/shared";

// (Typ-Definitionen jetzt in @ss/shared/src/config.ts -- siehe Re-Exports oben)
...
export function calcPriceDetailed(
  ...
  const resolveRegionValue = (map: Record<Region, number>) => {
    const region = normalizeRegion(cfg.region ?? "NRW");
